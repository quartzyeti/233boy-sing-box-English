...
in_conf() {
    ...
    [[ $? != 0 ]] && warn "Unable to read this file: $1" && return
    ...
    if [[ $host ]]; then
        ...
        vmess | vless)
            [[ $net_type == "http" ]] && {
                net=http
                is_tips_msg="New configuration file name: (VMess-HTTP-$port.json)"
            }
        ...
        dokodemo-door)
            add door
            is_tips_msg="New configuration file name: (Direct-$port.json)"
            ;;
        *socks*)
            add $is_protocol
            ;;
        *)
            is_not_in_conf=1
            msg "Import is not supported $1"
            ;;

        esac
    fi
    [[ ! $is_not_in_conf ]] && msg "Import data: $1 $is_tips_msg" && rm $1
}
is_change=1
is_dont_auto_exit=1
is_dont_test_host=1
...
if [[ ${is_list[@]} ]]; then
    msg "Start importing configuration..."
    for i in ${is_list[@]}; do
        in_conf $i &
    done
    wait
    ...
    err "No importable configurations were found..."
fi